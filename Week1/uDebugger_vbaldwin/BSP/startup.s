...
;       (NOTE that IAR needs the name to be  __vector_table for the debugger and C-Spy to work correctly)
      ...
      EXTERN  FaultPrint
      EXTERN  FaultPrintExceptionStack
...
HardFaultIrqHandler
       ...
       *    Print the entire stack frame consisting of R0-R3, R12, LR, PC, and PSR.
       */
      PUSH      {LR}        ; Save LR to stack since we will be calling a function below.
      MOV       R0, SP      ; move SP to R0 in preparation of the function call
      ADD       R0, R0, #4  ; add 4 bytes to SP to account for LR being the most recent push
      BL        FaultPrintExceptionStack
      ...
      ADD       R0, R0, #28 ; Add 7 word offset to R0 so it points to location where PC was pushed on stack.
      PUSH      {R0}        ; Save R0 to stack since we will want to reference it later
      MOV       R1, R0      ; Copy RO to R1
      SUB       R1, R1, #4  ; Subtract 4 from R1 so it points to the stacked LR value
